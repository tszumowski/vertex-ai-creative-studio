...
class FirestoreClient:
    ...
    def query(
        ...
        Args:
            collection_name: The name of the collection.
            query_fun: A function that takes a Firestore query object
                and returns a modified query (e.g., with where clauses,
                order_by, etc.)

        Returns:
            A list of document dictionaries that match the query. Returns
            an empty list if no documents match or raises exception if query
            fails.
        """
        try:
